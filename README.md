...
### 9. WebRTC接続テスト (`test-webrtc.html`)
- **🚀 低遅延モード**: 最高性能のWebRTC設定でのテスト
- **PeerJS接続テスト**: 基本的なPeer-to-Peer接続の確認
- **カメラ機能テスト**: 通常・低遅延カメラ設定の比較
- **通話機能テスト**: 通常・低遅延通話の品質比較
- **統計情報監視**: リアルタイムでの詳細な接続統計
- **デバッグログ**: 詳細なログ出力で問題の特定
- **設定比較**: 通常モードと低遅延モードの性能差を確認
...
3. **🚀 低遅延テスト機能**:
   - 配信IDを入力後、「🚀 低遅延テスト」ボタンをクリック
   - 自動的に遅延を0秒に設定し、ゼロ遅延モードで接続
   - 最高性能の映像配信を実現（60fps、3Mbps対応）
   - GPU加速とハードウェアアクセラレーションを使用
   - リアルタイムで性能統計を監視
...
6. **🧪 WebRTC接続テスト機能**:
   - 2つのタブまたはブラウザウィンドウで `test-webrtc.html` を開く
   - 片方で「🚀 低遅延モード初期化」→「🚀 低遅延カメラ開始」
   - もう片方で「PeerJS初期化」→「カメラ開始」
   - IDをコピーして相互に接続テスト
   - 「🚀 低遅延通話開始」で最高性能の通話を開始
   - 「統計開始」で詳細な接続統計を監視
   - 通常モードと低遅延モードの性能差を比較
...
- **WebRTC + PeerJS**: P2P映像配信（最大1080p 60fps、3Mbps）
- **MediaStreamTrackProcessor**: 高性能フレーム処理（Chrome/Edge/Safari対応）
- **HTML5 Canvas**: 遅延バッファリング（フォールバック機能）
- **GPU加速**: ハードウェアアクセラレーション（transform3d、willChange）
- **低遅延最適化**: ICE設定、エンコーディング設定、トランシーバー設定
