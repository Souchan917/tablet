...
3. **🚀 低遅延モード**:
   - 受信側で「🚀 低遅延モード」ボタンをクリック
   - 最高性能のゼロ遅延配信を実現
   - 60fps、3Mbps対応の高品質配信
   - GPU加速とハードウェアアクセラレーション使用
...
#### 従来の接続方法（バックアップ用）
配信側で配信IDをコピーして受信側に手動入力することも可能です。

### 特徴
- **完全自動接続**: 配信ID入力不要
- **固定ID使用**: 'tablet-broadcast-fixed' ⇄ 'tablet-viewer-fixed'
- **自動再接続**: 接続が切れても自動で再接続を試行
- **他者接続不可**: 固定IDのため、他の人が接続できない
- **一対一専用**: 1台の配信側と1台の受信側のみ対応
